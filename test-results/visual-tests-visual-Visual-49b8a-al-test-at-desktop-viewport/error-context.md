...
- Name: Visual Tests >> Tabs variation 1 visual test at desktop viewport
- Location: /Users/sselvara/adobe/adobe/Franklin/aem-visual-cheker/visual-tests/visual.spec.js:439:3
...
Call log:
  - navigating to "/tools/sidekick/library.html?plugin=blocks&path=/tools/sidekick/library/templates/tabs&index=1&vtest=true", waiting until "load"
    ...
    at /Users/sselvara/adobe/adobe/Franklin/aem-visual-cheker/visual-tests/visual.spec.js:444:16
```
...
> 444 |     await page.goto('/tools/sidekick/library.html?plugin=blocks&path=/tools/sidekick/library/templates/tabs&index=1&vtest=true');
  ...
  505 |       el.style.overflow = 'visible';
  506 |       el.style.maxHeight = 'none';
  507 |     }, block);
  508 |     
  509 |     // Get the bounding box of the block
  510 |     const box = await block.boundingBox();
  511 |     if (!box) throw new Error('Could not get bounding box for Tabs');
  512 |     
  513 |     // Take a screenshot of only the block area
  514 |     const screenshotName = 2 > 1 ? 'tabs-variation-1-large.png' : 'tabs-large.png';
  515 |     await expect(page).toHaveScreenshot(screenshotName, {
  516 |       clip: box,
  517 |       timeout: 30000,
  518 |       maxDiffPixels: 500,
  519 |       threshold: 0.4,
  520 |       animations: 'disabled',
  521 |       fullPage: box.height > 900
  522 |     });
  523 |   });
  524 | });
```