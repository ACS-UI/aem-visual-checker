...
- **Playwright-generated**: run `npm run test:visual:docker:update` once, review the output, commit it as the source of truth.
...
Specs aren't hand-written. `npm run test:visual:generate` runs [`generate-visual-tests.js`](generate-visual-tests.js), which:
...
- `POST /api/run-visual-test` — runs `npm run test:visual:docker:block -- tools/visual-tests/blocks/<component>` for the requested block (component name is validated against `^[a-z0-9-]+$`).
...
# in another terminal
npm run test:visual:docker:build   # build the image
npm run test:visual:docker         # run the suite in the container
npm run test:visual:docker:update  # run with --update-snapshots
...
3. Either:
   - run `npm run test:visual:docker:update` and review/commit the generated PNGs, or
   - add a `config.js` with `FIGMA_CONFIG` next to the new spec and run `npm run test:visual:figma` to pull baselines from Figma.
4. Run `npm run test:visual:docker` and confirm it's green before committing the snapshots.
...
- **Local pass, CI fail (or vice versa) on font/pixel differences** — make sure Docker is actually being used (`npm run test:visual:docker`); it's the same container image CI uses, so a bare/non-Docker run is unsupported and not expected to match.
- **Figma download fails** — check `FIGMA_ACCESS_TOKEN` is set and the `figmaUrl` in `config.js` contains a `node-id` query param.
- **Docker not running** — `npm run test:visual:docker*` and the pre-commit hook all require Docker to be installed and running; there is no local-CLI fallback.
